...
from .forms import NewMeetingForm, EntryTopicsForm, MultiUploadFileForm, EntryActivityForm, \
    MultiUploadActivityFileForm, TopicActivityForm, UpdateActivityForm, UpdateTopicsForm, NewExternalParticipantForm, \
    InputBulkUserForm
...
from .utils import create_pdf, send_pdf, send_notify
import pandas as pd
...
def dashboard_mom_detail(request, pk):
    ...
    topics = Topic.objects.filter(topic2meeting=meet).order_by('topic_name')
...
def edit_meeting(request, pk):
    meet = get_object_or_404(Meeting, pk=pk)
    if request.method == 'POST':
        form = NewMeetingForm(request.POST, instance=meet)
        if form.is_valid():
            form.save()
            messages.success(request, "Data New Meeting has been updated!")
            return redirect('pica:dashboard_mom_detail', pk)
    cpy = meet.meeting2company.short_code
    form = NewMeetingForm(instance=meet)
    context = {
        'form': form,
        'meet': meet,
        'cpy': cpy,
    }
    return render(request, 'pica/edit_meeting.html', context)
...
def search_topic(request):
    ...
    if request.GET.get('search_query'):
        search_query = request.GET.get('search_query')
    topiks = Topic.objects.filter(Q(status='O'),
                                  Q(topic_name__icontains=search_query) | Q(problem_info__icontains=search_query))
    page = request.GET.get('page')
    results = 20
    paginator = Paginator(topiks, results)
    try:
        topiks = paginator.page(page)
    except PageNotAnInteger:
        page = 1
        topiks = paginator.page(page)
    except EmptyPage:
    ...
    return render(request, 'pica/search_topic.html', context)


# Notify Topic
def notify_topic(request):
    today = datetime.today()
    topiks = Topic.objects.filter(status='O', due_date__lte=today)
    page = request.GET.get('page')
    results = 20
    paginator = Paginator(topiks, results)
    try:
    ...
    return render(request, 'pica/notify_topic.html', context)
...
def notify_all(request):
    ...
    if topiks:
        for topik in topiks:
            all_pics = topik.topic2user.all()
            if all_pics:
                for pic in all_pics:
                    if pic in all_pic_expired:
                        continue
                    else:
                        all_pic_expired.append(pic)
                        all_topics_user = pic.user2topic.filter(due_date__lte=today)
                        send_notify(request, all_topics_user, pic.email)
        messages.success(request, "All PIC have been notified.")
    return redirect('pica:notify_topic')
...
class NewExternalParticipantView(View):

    def get(self, request, pk):
        meet = get_object_or_404(Meeting, pk=pk)
        if not request.user.is_authenticated:
            return redirect('login')
        form = NewExternalParticipantForm()
        context = {
            'form': form,
            'meet': meet,
        }
        return render(request, 'pica/external_participant.html', context)

    def post(self, request, pk):
        meet = get_object_or_404(Meeting, pk=pk)
        form = NewExternalParticipantForm(request.POST, request.FILES)

        if form.is_valid():
            form.save()
            messages.success(request, "New external participant has been saved successfully!")
            return redirect('pica:input_outside_participant', meet.pk)
        else:
            print(form)
            print(form.errors)
...
class InputBulkUserView(View):
    def get(self, request):
        form = InputBulkUserForm()
        context = {
            'form': form,
        }
        return render(request, 'input_bulk_user.html', context)

    def post(self, request):
        form = InputBulkUserForm(request.POST or None, request.FILES or None)
        if form.is_valid():
            cd = form.cleaned_data
            df = pd.read_csv(cd['isi_file'])
            for index, row in df.iterrows():
                ...
                if User.objects.filter(username=username).first():
                    ...
                    continue
                    # messages.error(request, f"This username is already taken {username}")
                    # return redirect('home')
                else:
                    User.objects.update_or_create(username=username, first_name=first_name, last_name=last_name,
                                                  email=email,
                                                  password=password)
            messages.success(request, "Input Bulk User berhasil")
            return redirect('home')
        else:
            print(form.errors)
            print(form.fields)
